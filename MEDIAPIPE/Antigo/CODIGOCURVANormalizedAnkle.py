...
df = pd.DataFrame(data, columns=["time_sec", "ankle_angle", "ankle_dist"])

#aplicar filtro
df["ankle_smooth"] = butter_lowpass_filter(df["ankle_angle"])
...
for i in range(len(peaks) - 1):
    ...
    ankle_stride = df["ankle_smooth"].iloc[start:end].values

    if len(ankle_stride) < 10:   #filtrar zancadas mt pequenas (invalidas)
        continue

    ankle_norm = normalize(ankle_stride)

    ankle_cycles.append(ankle_norm)   #guardar todas as zancadas (normalizadas)

ankle_cycles = np.array(ankle_cycles)

ankle_mean = np.mean(ankle_cycles, axis=0)    #media
ankle_std = np.std(ankle_cycles, axis=0)  #desvio padrao
...
plt.plot(x, ankle_mean, label="Mean ankle", color="blue")     #GRAFICO FINAL
plt.fill_between(
    x,
    ankle_mean - ankle_std,   #Linha azul central
    ankle_mean + ankle_std,   #Area azul de cada zancada individual
    alpha=0.2,
    color="blue"
)
